(ql:quickload '(:drakma :cl-async :cl-json :websocket-driver-client) :silent t)
...
(defvar *ws-url* (get-slack-info))
(defvar *client* (wsd:make-client *ws-url*))

(defun send-slack-message (message channel)
  (wsd:send *client* (json:encode-json-to-string 
...
(defun receive-data (data)
  (destructuring-bind (type channel message) (parse-slack-data data)
    (if (was-dad-mentioned? type message) (send-joke channel))))

(wsd:on :message *client* #'receive-data)
